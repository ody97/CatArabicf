...
async def get_anime_schedule(weekid):
    "احصل على جدول الأنيمي"
    dayname = get_weekday(weekid)
    result = f"✙ **الرسوم الكرتونية المجدولة لـ {dayname.title()} are : **\n\n"
...
async def formatJSON(outData):
    ...
    for g in jsonData["genres"]:
        msg += g + " "
    msg += f"\n**▾∮ حالة** : {jsonData['status']}"
    msg += f"\n**▾∮ حلقة** : {jsonData['episodes']}"
    msg += f"\n**▾∮ سنة** : {jsonData['startDate']['year']}"
    msg += f"\n**▾∮ نتيجة** : {jsonData['averageScore']}"
    msg += f"\n**▾∮ مدة** : {jsonData['duration']} min\n\n"
...
** ✙ احصائيات الانمي **
• ** إجمالي مشاهدة الأنمي :** `{user_data["statistics"]["anime"]['count']}`
• ** مجموع الحلقة المشاهدة : **`{user_data["statistics"]["anime"]['episodesWatched']}`
• ** إجمالي الوقت المنقضي : **`{readable_time(user_data["statistics"]["anime"]['minutesWatched']*60)}`
• **متوسط النتيجة :** `{user_data["statistics"]["anime"]['meanScore']}`
** ✙ إحصائيات مانغا **
• ** إجمالي قراءة المانجا:** `{user_data["statistics"]["manga"]['count']}`
• ** مجموع الفصول المقروءة:** `{user_data["statistics"]["manga"]['chaptersRead']}`
• ** إجمالي مجلدات القراءة: **`{user_data["statistics"]["manga"]['volumesRead']}`
• **متوسط النتيجة : **`{user_data["statistics"]["manga"]['meanScore']}`
"""
    )
...
async def get_anime_manga(mal_id, search_type, _user_id):  # sourcery no-metrics
    ...
    if search_type == "anime_anime":
        ...
        if trailer:
            TRAILER = f"<a href='{trailer}'>🎬 مقطع دعائي</a>"
        else:
            TRAILER = "🎬 <i>لا يوجد مقطع دعائي متاح</i>"
        studio_string = ", ".join(
    ...
    if alternative_names:
        ...
        caption += f"\n<b>يُعرف أيضًا باسم </b>: <i>{alternative_names_string}</i>"
    ...
    if result["synopsis"] is not None:
        ...
        synopsis_string = "غير معروف"
    for entity in result:
        if result[entity] is None:
            result[entity] = "غير معروف"
    if search_type == "anime_anime":
    ...
    if search_type == "anime_anime":
        ...
        🆔 <b>معرف MAL</b>: <i>{result['mal_id']}</i>
        📡 <b>الحالة</b>: <i>{result['status']}</i>
        🎙️ <b>الاذاعة</b>: <i>{result['aired']['string']}</i>
        🔢 <b>الحلقات</b>: <i>{result['episodes']}</i>
        🔞 <b>تقييم</b>: <i>{result['rating']}</i>
        💯 <b>النتيجة</b>: <i>{result['score']}</i>
        🌐 <b>عرض لاول مرة</b>: <i>{result['premiered']}</i>
        ⌛ <b>المدة</b>: <i>{result['duration']}</i>
        🎭 <b>الانواع</b>: <i>{genre_string}</i>
        🎙️ <b>استوديوهات</b>: <i>{studio_string}</i>
        ...
        caption += f"<b>{TRAILER}</b>\n📖 <a href='{synopsis_link}'><b>ملخص</b></a> <b>&</b> <a href='{result['url']}'><b>اقرا المزيد</b></a>"
    elif search_type == "anime_manga":
        caption += textwrap.dedent(
            f"""
        🆎 <b>النوع</b>: <i>{result['type']}</i>
        📡 <b>الحالة</b>: <i>{result['status']}</i>
        🔢 <b>الاحجام</b>: <i>{result['volumes']}</i>
        📃 <b>فصول</b>: <i>{result['chapters']}</i>
        📊 <b>مرتبة</b>: <i>{result['rank']}</i>
        💯 <b>نتيجة</b>: <i>{result['score']}</i>
        🎭 <b>Genres</b>: <i>{genre_string}</i>
        📖 <b>ملخص</b>: <i>{synopsis_string}</i>
        """
        )
