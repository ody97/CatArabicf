...
def errors_handler(func):
    async def wrapper(errors):
        try:
            ...
            ftext = f"\nإخلاء المسؤولية: \ n يتم لصق هذا الملف هنا فقط هنا ،\
                                  \nقمنا بتسجيل حقيقة الخطأ والتاريخ فقط ، \n نحن نحترم خصوصيتك ،\
                                  \nلا يحق لك الابلاغ عن الخطأ اذا كنت \
                                  \nاي بيانات سرية ، لا يمكن لحد ان يراها\
                                  \n\n--------بدء سجل تتبع USERBOT--------\
                                  \التاريخ: {date}\nايدي المجموعة: {str(check.chat_id)}\
                                  \nهوية المرسل: {str(check.sender_id)}\
                                  \n\nمشغل الحدث:\n{str(check.text)}\
                                  \n\nمعلومات تتبع الاثر:\n{str(traceback.format_exc())}\
                                  \n\nنص الخطأ:\n{str(sys.exc_info()[1])}"
            new = {
                "خطا": str(sys.exc_info()[1]),
                "التاريخ": datetime.datetime.now(),
            }

            ftext += "\n\n--------سجل تتبع USERBOT النهائي--------"
            ...
            text = "**أعد تقرير خطا CatArabic**\n\n"
            link = "[اضغط](https://t.me/CatArabic)"
            text += "اذا كنت تريد الحل فيمكنك الابلاغ عن الخطأ"
            text += f"فقط قم بإعادة توجيه هذه الرسالة {link}.\n"
            text += "لا يتم تسجيل اي شي الا حقيقة الخطأ والتاريخ\n\n"
